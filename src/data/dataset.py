...
class TimeFrequencyMask(Processor):
    def __init__(self, keys: list[str] = ["audio"], time_mask_param:int = 100, time_p = 0.8, freq_mask_param:int = 100):
...
class Truncate(Processor):
    ...
    def _process(self, x, key = "audio"):
        audio = x.get(key)
        random_truncate = torch.randint(0, audio.shape[0] - self.n_samples, (1,))
        audio = audio[random_truncate:random_truncate + self.n_samples]
        return {
            key: audio
        }
...
class MelSpectrogram(Processor):
    ...
    def __init__(self, sample_rate: int = 16000, n_fft: int = 400, hop_length: int = 160, n_mels: int = 128, 
                 f_min: float = 0.0, f_max: float = None, win_length: int = None, power: float = 2.0, keys: list[str] = ["audio"]):
        super().__init__(keys = keys)
...
class MultiView(Processor):
    def __init__(self, view_samples, strategy: str = "same_view", keys: list[str] = ["audio"]):
        super().__init__(keys = keys)
    ...
    def _get_adjacent_view(self, audio):
        start_idx = torch.randint(0, audio.shape[0] - self.view_samples, (1,))
        view_1 = audio[start_idx:start_idx + self.view_samples]
        view_2 = audio[start_idx + self.view_samples:start_idx + self.view_samples * 2]
